...
describe('Popup Component', () => {
  ...
  beforeEach(() => {
    ...
    URL.createObjectURL = vi.fn().mockReturnValue('blob:http://localhost/mocked-object-url');
  ...
  it('should display an error message if fetchTTS fails', async () => {
    vi.mocked(ttsProviders.fetchTTS).mockRejectedValue(new Error('TTS API error from test'));

    render(<Popup />);
    const generateButton = screen.getByRole('button', { name: /音声を生成/i });
    fireEvent.change(screen.getByRole('textbox'), { target: { value: 'エラーテスト' } });
    fireEvent.click(generateButton);
